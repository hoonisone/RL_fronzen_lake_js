...
#             좌표가 맵을 벗어났는지 체크
#         """
#         return FrozenLake.IS_OUT(self.map, x, y)
...
#         return FrozenLake.COORDINATE_TO_STATE(self.map_size, x, y)
    
#     @staticmethod
#     def COORDINATE_TO_STATE(map_size, x, y):
#         return map_size * y + x
...
#         changed_state_action_pairs = ChangingFrozenLake.get_changed_state_action_pair(self.map, new_map)
#         self.map = new_map
...
#             for action, [dx, dy] in FrozenLake.action_to_direction_dict.items():
#                 if not FrozenLake.IS_OUT(map1, x-dx, y-dy):
#                     changed_state_action_pairs.append((FrozenLake.COORDINATE_TO_STATE(map1.shape[0], x-dx, y-dy), action))
...
# print(ChangingFrozenLake.get_all_map_name())